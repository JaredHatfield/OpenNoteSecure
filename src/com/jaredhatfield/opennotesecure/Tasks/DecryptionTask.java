 ...
 */
public class DecryptionTask extends
    ...
    protected FileTaskHolder doInBackground(FileTaskHolder... arg0) {
        ...
        if (holder.getEncryption().equals("None")) {
            ...
            String ciphertext = FileManager.Instance().readFile(
                    holder.getFile());
            if (ciphertext.length() > 0) {
                ...
                // The file is blank so we set the result to indicate the
            ...
            if (ciphertext.length() > 0) {
                try {
                    DESEncryptionProvider des = new DESEncryptionProvider(
                            holder.getPassword());
